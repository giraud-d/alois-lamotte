<script lang="ts">
    ...
    import {paintings, type Painting} from '$lib/stores/paintings';

    const t = derived(language, () => translations[$language]);

    interface PaintingsByYear {
        [key: number]: Painting[];
    }

    const paintingsByYear: PaintingsByYear = paintings.reduce<PaintingsByYear>((acc, painting) => {
    ...
    Object.keys(paintingsByYear).forEach(year => {
        paintingsByYear[Number(year)].sort((a, b) =>
            new Date(b.date).getTime() - new Date(a.date).getTime()
        );
    });
</script>

<h1 class="py-2 text-center text-5xl font-medium">{$t.painting.title}</h1>

{#each Object.keys(paintingsByYear).sort((a, b) => Number(b) - Number(a)) as year}
    <div class="mb-16">
        ...
        <div class="max-w-[1800px] mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
                {#each paintingsByYear[Number(year)] as painting}
                    <div class="flex items-center justify-center w-full">
                        <a href="/painting/{painting.link}"
                           class="relative w-full cursor-pointer bg-transparent border-0 p-0 block"
                           role="button"
                           tabindex="0">
                            <div class="svgBlob" style="aspect-ratio: {Math.min(2, painting.dimensions.width / painting.dimensions.height)}/1;">
                                <svg viewBox="0 0 500 500" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">
                                    <defs>
                                        <linearGradient id="blobGradient-{year}-{painting.title}">
                                            <stop offset="0%" stop-color="#fff"></stop>
                                            <stop offset="100%" stop-color="#fff"></stop>
                                        </linearGradient>
                                    </defs>
                                    <mask id="blobMask-{year}-{painting.title}">
                                        <rect></rect>
                                        <path>
                                    ...
                                    <rect x="-1" y="-1" width="502" height="502"
                                          mask="url('#blobMask-{year}-{painting.title}')"
                                          fill="url('#blobGradient-{year}-{painting.title}')"></rect>
                                </svg>
                                <img
                                        src={painting.preview}
                                        alt={painting.title}
